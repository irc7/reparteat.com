...
require_once("../../../includes/class/Product/class.Product.php");	
	...
	if($_POST) {		
		...
		if($action == "delete") {
			...
			$newUd = intval($_POST["totalPro"]);
			if($newUd == 0) {
				unset($carrito["data"][$indice]);
			}else{
				$cost_unidad = $carrito["data"][$indice]["cost"] / $carrito["data"][$indice]["ud"];
				$carrito["data"][$indice]["ud"] = $newUd;
				$carrito["data"][$indice]["cost"] = number_format(($cost_unidad * $newUd),2,".","");
			}
