<?php if($order->IDMETHODPAY == 1 && ($order->STATUS == 2 || $order->STATUS == 3) && ($orderObj->checkingProveedorOrder($_SESSION[nameSessionZP]->ID, $order->IDSUPPLIER) || $orderObj->checkViewOrderZone($_SESSION[nameSessionZP], $order))){ 
		$products = $proObj->listProductBySupplier($supplierCart->ID);
	//	pre($products);
?>

	<button type="button" id="btn-add-product-order-<?php echo $refSend; ?>" class="btn btn-primary floatRight transition btn-add-product-order">Añadir producto</button>
		...
		<div class="container">
			<div class="row">
				<div class="wrap-msg-alert text-left">
					...
					<div class="separator5"></div>
					<form method="post" action="<?php echo DOMAINZP; ?>editar-pedido" name="add-product" id="add-product">
						...
						<div class="col-xs-12">
							<button id="btn-add-product" type="submit" class="btn btn-primary floatRight transition btn-send-order" disabled>CONTINUAR</button>
						</div>
						<div class="separator5"></div>
					</form>
