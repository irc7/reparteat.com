<?php
	...
	if($_GET) {
		...
		if($idUser == $_SESSION[nameSessionZP]->ID) {
			$addressObj = new Address();

			if($addressObj->deleteAddressUser($idAddress, $idUser, $idZone)) {
				$msg = "Dirección eliminada correctamente.";
			}else{
				$msg = "Se ha producido un error al borrar la dirección, por favor vuelva a intentarlo, si el problema persiste consulte con el administrador.";
			}
		}else{
			$error = 1;
			$msg.= NOACCESS;
