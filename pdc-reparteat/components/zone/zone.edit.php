<?php if (allowed ($mnu)){ ?>
<div class='cp_mnu_title title_header_mod'>Editar zona</div>
<?php
	if (isset($_GET['msg'])) {
...
<?php
	...
	$zoneObj = new Zone();
	$zone = $zoneObj->infoZone($id); 
	$userObj = new UserWeb();
	$userRes = array(); 
	$userRes = $userObj->listUserWebByType(5); 

	$idRes = $zoneObj->infoZoneUser($id);
?>	
	...
	<form method='post' action='modules/zone/edit.php' enctype='multipart/form-data' id='mainform' name='mainform'>
			...
			<div class="col-sm-6 col-xs-12">
				<div class="form-group">
					...
					<select class="form-control form-s" name="status" id="status" title="Estado">
						<option value='1'<?php if($zone->STATUS == 1){ echo ' selected="selected"';} ?>>Activado</option>
						<option value='0'<?php if($zone->STATUS == 0){ echo ' selected="selected"';} ?>>Desactivado</option>
					</select>
				</div>
			</div>
			<div class="col-sm-6 col-xs-12">
				<?php $typeZones = $zoneObj->listTypeZone(); ?>
				<div class="form-group">
					...
					<?php foreach($typeZones as $t) { ?>}	
						<option value='<?php echo $t; ?>'<?php if($t == $zone->TYPE){echo " selected";} ?>><?php echo $t; ?></option>
					<?php } ?>
					</select>
				</div>
			</div>
		...
		<div class='container container-admin'>
			<div class='row'>	
				<div class="form-group">
					...
					<input type="text" name="City" id="City" class="form-control form-s" title="Ciudad" placeholder="Ciudad *" required value="<?php echo $zone->CITY; ?>" />
					<p id="error-City"></p>
				</div>
				<div class="form-group">
					<label class="label-field" for="CP">Código postal *:</label>
					<input type="text" name="CP" id="CP" class="form-control form-xs" title="Código postal" placeholder="Código postal *" required value="<?php echo $zone->CP; ?>" />
					<p id="error-CP"></p>
				</div>
				<div class="form-group">
					...
					<?php 
						...
						while($row = mysqli_fetch_object($rP)) {
							?>
						<option value="<?php echo $row->PROVINCE; ?>"<?php if($row->PROVINCE === $zone->PROVINCE){echo " selected";} ?>>
				...
				<div class="form-group" style="display:none;">
					...
					<input type="number" name="RepLimit" id="RepLimit" class="form-control form-xs" title="Límite repartidores" placeholder="Límite repartidores" step="1"  disabled value="<?php echo $zone->REP_LIMIT; ?>"/>
					<p id="error-RepLimit"></p>
				</div>
				<div class="form-group">
					<label class="label-field" for="Shipping">Gastos de envío *:</label>
					<input type="number" name="Shipping" id="Shipping" class="form-control form-xs" title="Gastos de envio" placeholder="Gastos de envío *" step="0.01" value="<?php echo $zone->SHIPPING; ?>" />
					<p id="error-Shipping"></p>
				</div>
				<div class="form-group">
					<label class="label-field" for="time_delivery">Tiempo estimado para el reparto *:</label>
					<input type="number" name="time_delivery" id="time_delivery" class="form-control textRight form-xs" title="Tiempo estimado para el reparto" placeholder="20 *" required step="1" value="<?php echo $zone->TIME_DELIVERY; ?>" />
				...
				<div class="form-group">
					...
					<input type="number" name="time_check_order" id="time_check_order" class="form-control textRight form-xs" title="Tiempo de chequeo de pedidos" placeholder="7 *" required step="1" value="<?php echo $zone->TIME_CHECK_ORDER; ?>" />
				...
				<div class="form-group">
					...
					<input type="number" name="time_orders_zones" id="time_orders_zones" class="form-control textRight form-xs" title="Franjas horarias" placeholder="1200 *" required step="1" value="<?php echo $zone->TIME_ORDERS_ZONES; ?>" />
			...
			<?php foreach($userRes as $us) { ?>
				<div class="col-md-4 col-xs-6 col-sm-12">
					<input type="checkbox" name="Responsable[]" title="Responsable" value="<?php echo $us->ID; ?>" <?php for($i=0;$i<count($idRes);$i++){if($idRes[$i] == $us->ID){echo " checked";}} ?> />
		...
		<div class='container container-admin'>
			<div class='row'>	
				...
				<div class='col-md-5'>
					<input class="btn tf-btn btn-default transition floatRight bgGreen white bold" type='submit' name='save' value='GUARDAR ZONA' />
				</div>
			</div>
		</div>
	</form>
