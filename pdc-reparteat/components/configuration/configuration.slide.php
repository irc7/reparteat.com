<?php if (allowed($mnu)) { ?>
	<div class='cp_mnu_title title_header_mod'>Banners rotativos</div>
	<div class="cp_box">
		<?php 
		if (isset($_GET['msg'])) {
		...
		<?php } ?>
		<p>Establezca aquí los valores predeterminados con los que se crearán los banners rotativos. Posteriormente, podrá editar dichos valores desde <a style="color:#2f81bb;" href="<?php echo DOMAIN;?>pdc-reparteat/index.php?mnu=design&com=slide&tpl=option&opt=section">aquí</a>.</p>
      	<br/>
		<?php 
		//$q = "describe spl_slider_gallery where Field = 'HEIGHT' or Field = 'WIDTH'"; 
		$q = "show columns from ".preBD."slider_gallery where Field = 'HEIGHT' or Field = 'WIDTH'";
	...
	?>
		...
		<form method='post' action='modules/configuration/change_style_slider.php' enctype='multipart/form-data' id='mainform' name='mainform'>
			<div class='cp_formfield bold'>
				<label for='alto'>Alto de banner:</label>
			</div>
			...
			<div class='cp_formfield bold'>
				<label for='ancho'>Ancho de banner:</label>
			</div>
			<div class='cp_table350'>
