...
?>		
				<div class='col-md-12 shaded item-list'>
					...
					<?php 
						$urlAlert = "modules/product/action_cat.php?".$urlMod."&action=delete&id=".$catList[$i]->ID;
						$msgAlert = "¡ATENCI&Oacute;N! Va a eliminar la categoría de producto ".$catList[$i]->TITLE." y sus asociaciones a los mismos. &iquest;Est&aacute; seguro?";
					?>
						<div class='col-xs-6'>
				...
				<div id="editcat-<?php echo $catList[$i]->ID; ?>" class="form-category">
					<form method='post' action='modules/product/edit_category.php' enctype='multipart/form-data' id='mainform-<?php echo $catList[$i]->ID; ?>' name='mainform'>
						<input type="hidden" name="mnu" value="<?php echo $mnu; ?>" />
						<input type="hidden" name="com" value="<?php echo $com; ?>" />
						<input type="hidden" name="tpl" value="<?php echo $tpl; ?>" />
						<input type="hidden" name="opt" value="<?php echo $opt; ?>" />
