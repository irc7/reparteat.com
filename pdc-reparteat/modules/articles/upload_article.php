<?php
	...
	/*COMPARARACION DE TABLA DE ARTICULO*/	
		...
		if($article["TITLE"] != $temp["TITLE"]){
			$title = mysqli_real_escape_string($connectBD,$temp["TITLE"]);
			$change_title = TRUE;
		} else {
			$change_title = FALSE;
		}
		...
		$title_seo = mysqli_real_escape_string($connectBD,$temp["TITLE_SEO"]);
		
		if($article["SUBTITLE"] != $temp["SUBTITLE"]){
			$subtitle = mysqli_real_escape_string($connectBD,$temp["SUBTITLE"]);
		...
		if($article["SUMARY"] != $temp["SUMARY"]){
			$sumary = mysqli_real_escape_string($connectBD,$temp["SUMARY"]);
			$change_sumary = TRUE;
		} else {
			$change_sumary = FALSE;
		}
		...
		if($article["INTRO"] != $temp["INTRO"]){
			$intro = mysqli_real_escape_string($connectBD,$temp["INTRO"]);
			$change_intro = TRUE;
		} else {
			$change_intro = FALSE;
		}
		
		if($article["DATE_START"] != $temp["DATE_START"]){
		...
		if($article["DATE_END"] != $temp["DATE_END"]){
	...
	/*COMPARACION DE PARRAFOS*/	
		...
		while($row_temp = mysqli_fetch_assoc($result_temp)) {
			...
			$block_title = mysqli_real_escape_string($connectBD,$row_temp["TITLE"]);
			$block_text = stripslashes($row_temp["TEXT"]);
			$block_foot = mysqli_real_escape_string($connectBD,$row_temp["FOOT"]);
			...
			if ($row_article["IMAGE"] != "" && $row_article["IMAGE"] != NULL) {
				...
				$urlnewsletter = "../../../files/articles/newsletter/".$row_article["IMAGE"];
				$urlThumb = "../../../files/articles/thumb/".$row_article["IMAGE"];
				deleteFile($urlImg);
				deleteFile($urlnewsletter);
				deleteFile($urlThumb);
			}
			...
			$q_up .= " TITLE = '" . $block_title;
			$q_up .= "', ALIGN = '". $block_align;
			$q_up .= "', FOOT = '" . $block_foot;
			...
			while($files = mysqli_fetch_object($result_file)){
				if ($files->IDCOPY == 0 && $files->URL != "") {
					...
					$q4 .= "('".$pUp->ID."', '".$files->TITLE."', '".$files->URL."', '".$files->SIZE."')"; 
					...
					$q5 = "UPDATE ".preBD."paragraphs_file SET IDPARAGRAPH = " . $pUp->ID . " where ID = " . $files->IDCOPY; 
	...
	//BORRADO DE IMÁGENES
		for($j=0;$j<$cont;$j++) {
			if(($temp_opt[$j]["type"] == "image") || (($temp_opt[$j]["type"] == "youtube") && ($temp_opt[$j]["image"] != $temp_opt[$j]["video"]))) {
				...
				$destino_newsletter = "../../../files/articles/newsletter/".$temp_opt[$j]["image"];
				$destino_thumb = "../../../files/articles/thumb/".$temp_opt[$j]["image"];
				
				$origen_img = "../../../files/articles_temp/image/".$temp_opt[$j]["image"];
				$origen_newsletter = "../../../files/articles_temp/newsletter/".$temp_opt[$j]["image"];
				...
				if($temp_opt[$j]["image"] != "") {
					...
					if(!file_exists($destino_newsletter) && file_exists($origen_newsletter)) {
					...
					$urlnewsletter = "../../../files/articles_temp/newsletter/".$temp_opt[$j]["image"];
		...
		$url_newsletter = "../../../files/articles/newsletter/".$img_art;
		...
		if (($img_art != "") && ($thumbYoutube == false)) {
			...
			$destino_newsletter = "../../../files/articles/newsletter/".$img_art;
			$destino_thumb = "../../../files/articles/thumb/".$img_art;
			$origen_img = "../../../files/articles_temp/image/".$img_art;
			$origen_newsletter = "../../../files/articles_temp/newsletter/".$img_art;
	...
	/*CONSULTAS*/
		...
		if(isset($_GET["urlart"]) && trim($_GET["urlart"]) != "") {
			$slug = formatNameUrl(trim($_GET["urlart"]));
		}else {
		...
		$q = "UPDATE `".preBD."url_web` SET 
			...
			WHERE ID_VIEW = '" . $id . "' and TYPE = 'article'";
		checkingQuery($connectBD, $q);
		
	//Noticias relacionadas
		$q = "DELETE FROM `".preBD."articles_related` WHERE (ID1 = " . $id ." or ID2 = " . $id .")";
		checkingQuery($connectBD, $q);
	...
	//FIN   
		...
		$location = "Location: ../../index.php?mnu=".$mnu."&com=articles&tpl=edit&record=".$id."&type=".$typeArticle."&msg=".utf8_decode($msg);
		header($location);
	} else {
		disconnectdb($connectBD);
