	...
	if (allowed($mnu)) {
			...
			switch ($option) {
				...
				case 1:
					...
					$q = "DELETE FROM ".preBD."articles_related WHERE ID1 = '" . $article . "' or ID2 = '" . $article . "'";
					checkingQuery($connectBD, $q);
					...
					$msg = "Artículo ".$article." eliminado definitivamente";
			}
			...
			if($section != 0) {
				...
				$q = "DELETE FROM `".preBD."url_web` WHERE ID_VIEW = '" . $article . "' and TYPE = 'article'";
				checkingQuery($connectBD, $q);
			}
			//FIN
		...
		if ($trash == 1) {
			...
			$location = "Location: ../../index.php?mnu=".$mnu."&com=articles&tpl=option&type=".$typeArticle."&msg=".utf8_decode($msg);
		}
		header($location);
	} else {
		disconnectdb($connectBD);
