	...
	if($_GET) {
		...
		if(!isset($_GET["mnu"]) || !allowed($_GET["mnu"])) { 	
			...
			if (isset($_GET["id"]) && $id > 0) {
				...
				switch($action) {
					case "nov":
						$q="UPDATE ".preBD."downloads SET NOVELTIES = '1' WHERE ID = '" . $id . "'";
						checkingQuery($connectBD, $q);
						
						$msg = "Descarga <em>".$down->TITLE."</em> activada para novedades.";
						
					break;
					case "unnov":
						$q="UPDATE ".preBD."downloads SET NOVELTIES = '0' WHERE ID = '" . $id . "'";
						
						checkingQuery($connectBD, $q);
						$msg = "Descarga <em>".$down->TITLE."</em> desactivada para novedades.";
