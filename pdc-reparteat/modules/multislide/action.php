<?php
	...
	if($_GET) {
		...
		if(!isset($_GET["mnu"]) || !allowed($_GET["mnu"])) { 	
			...
			$pObj = new Multislide();
			$pBD = $pObj->infoMultislideById($id);
			
			if($id >= 0) {
				switch($action) {
					case "publish":
						$pObj->upStatus($id, 1);
						$msgAlert = "Slide <em>".$pBD->TITLE."</em> publicado correctamente.";
					break;
					case "unpublish":
						$pObj->upStatus($id, 0);
						$msgAlert = "Slide <em>".$pBD->TITLE."</em> desactivado correctamente.";
					break;
					case "delete":
						$pObj->delete($pBD);
						$msgAlert = "Slide <em>".$pBD->TITLE."</em> eliminado correctamente.";
					break;
				}
			}
			disconnectdb($connectBD);
			$msg = $msgAlert;
			$location = "Location: ../../index.php?mnu=".$mnu."&com=".$com."&tpl=option&opt=".$opt.$urlAux."&msg=".utf8_decode($msg);
			header($location);
		}	
	}else {
		disconnectdb($connectBD);
