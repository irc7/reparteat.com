		...
		if (isset($_GET["record"])) {
			...
			while ($row_paragraphs = mysqli_fetch_object($result_paragraphs)) {			
				...
				while($files = mysqli_fetch_object($result_file)){
					...
					if($files->TITLE != ""){
						$title = "Copia de ".$files->TITLE;
					}
					
					$q4 = "INSERT INTO ".preBD."paragraphs_file (`IDPARAGRAPH`, `TITLE`, `URL`, `SIZE`, `POSITION`) VALUES";
					$q4 .= "('".$record_paragraph."', '".$title."', '".$new_doc."', '".$files->SIZE."', '".$files->POSITION."')"; 
			...
			$msg .= "Post duplicado correctamente";
			$location = "Location: ../../index.php?mnu=blog&com=blog&tpl=edit&record=".$record_number."&msg=".utf8_decode($msg);
			header($location);
		}
	} else {
		disconnectdb($connectBD);
